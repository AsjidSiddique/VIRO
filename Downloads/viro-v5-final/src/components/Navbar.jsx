...
export default function Navbar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex flex-col fixed left-0 top-0 h-full w-20 z-40 items-center py-6 gap-4"
          ...
          {NAV.map(n => {
            ...
            return (
              <Link key={n.path} to={n.path}
                className="flex flex-col items-center gap-1 p-3 rounded-xl transition-all text-xs font-semibold"
                style={active
                  ? { background:'linear-gradient(135deg,#00BFFF15,#8B5CF630)', color:'#A78BFA', border:'1px solid #8B5CF640' }
                  : { color:'#475569' }}>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex"
        ...
        {NAV.map(n => {
          const active = location.pathname === n.path
          return (
            <Link key={n.path} to={n.path} className="flex-1 flex flex-col items-center py-2 gap-0.5 relative">
              <span className="text-xl relative">
                {n.icon}
                {n.label === 'Cart' && cartCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full text-white flex items-center justify-center font-bold"
              ...
              <span className="text-xs font-semibold" style={{ color: active ? '#A78BFA' : '#475569' }}>
                {n.label}
              </span>
              {active && (
